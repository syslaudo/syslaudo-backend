...
const schemaMedico = yup.object().shape({
  ...
  emailUsuarioSolicitado: yup.string().email("Insira um email valido! Ex:exemple@exemple.com").required("Email é requerido!"),
});
...
class MedicoController {
  async create(req: Request, res: Response) {
    try {
      ...
      const userExists = await repository.findOne({ where: { email_usuario:  emailUsuarioSolicitado } });
      ...
      const medicoRelacao = await repositoryMedico.find({ relations: ['usuarios.id']   })
